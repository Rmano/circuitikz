...
\def\pgf@circ@bipole@path#1#2{
    ...
    \pgfextra{
        ...
        \pgfmathsetmacro\pgf@circ@direction{round(\pgfmathresult-\rot)}%Calculate direction(angle) of path
        \pgfsyssoftpath@setcurrentpath{\myp@th}
        \typeout{DIRECTION\space\pgf@circ@direction\space A:\a\space B:\b\space C:\c\space D:\d}
        \pgfmathsetmacro{\@@yscale}{\d*\ctikzvalof{mirror value}}
        \pgfmathsetmacro{\@@xscale}{\a*\ctikzvalof{invert value}}
        \pgfmathsetmacro{\@@invert}{ifthenelse(\@@xscale<0,1,0)}
        \typeout{\space\space XSCALE=\@@xscale\space YSCALE=\@@yscale\space INVERT=\@@invert}
    }
    ...
    \ifx\pgf@temp\pgf@circ@temp  % if it is an open
        ...
        node[#1shape, rotate=\pgf@circ@direction, 
            yscale=\ctikzvalof{mirror value}, xscale=\ctikzvalof{invert value},
